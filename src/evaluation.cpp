...
void Evaluation::ekfCallback(const nav_msgs::Odometry::ConstPtr& ekf)
{
  ...
  actual_data(0,3) = 0;
  actual_data(0,4) = index_ekf;
  position_ekf = join_vert(position_ekf, actual_data);

  actual_data(0,0) = ekf->pose.pose.orientation.x;
  actual_data(0,1) = ekf->pose.pose.orientation.y;
  actual_data(0,2) = ekf->pose.pose.orientation.z;
  actual_data(0,3) = ekf->pose.pose.orientation.w;
  actual_data(0,4) = index_ekf;
...
void Evaluation::viconCallback(const nav_msgs::Odometry::ConstPtr& vicon)
{
  if (index_ekf == index_vicon+1) {
    ...
    actual_data(0,3) = 0;
    actual_data(0,4) = index_vicon;
    position_vicon = join_vert(position_vicon, actual_data);

    actual_data(0,0) = vicon->pose.pose.orientation.x;
    actual_data(0,1) = vicon->pose.pose.orientation.y;
    actual_data(0,2) = vicon->pose.pose.orientation.z;
    actual_data(0,3) = vicon->pose.pose.orientation.w;
    actual_data(0,4) = index_vicon;
    ...
    actual_data(0,3) = 0;
    actual_data(0,4) = index_vicon;  actual_data(0,4) = index_ekf;
  ang_vel_ekf = join_vert(ang_vel_ekf, actual_data);
